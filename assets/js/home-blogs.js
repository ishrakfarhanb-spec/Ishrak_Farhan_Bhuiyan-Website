...
(function () {
  ...
  function toAbsoluteImg(url) {
    if (!url) return blogBase + 'assets/img/placeholder-wide.svg';
    // If already absolute (http/https), return as-is; otherwise prefix with blogBase
    return /^https?:\/\//i.test(url) ? url : (blogBase + url.replace(/^\/?/, ''));
  }

  function toCard(item) {
    const img = toAbsoluteImg(item.image);
  ...
  async function init() {
    ...
    try {
      ...
      el.innerHTML = '<p class="muted">Could not load latest blogs. ' +
        '<a href="' + blogBase + '" target="_blank" rel="noopener">Visit the blog →</a></p>';
    }
  }
